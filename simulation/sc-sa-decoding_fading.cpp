...
void structure_frame(vector<Frame>& str_frame, double g)
{
    ...
    for(int i=0; i<sc; i++)
    {
        ...
        str_frame[i].add_time_slot(number_segment);
        str_frame[i].add_device(n_device);
        for(int j=0; j<number_segment; j++) str_frame[i].add_slot();
    }
}
                    ...
                    for(int m=sc_frame[i].f_slot[j].s_edge.size(); m>0; m--)
                    {
                        for(list<Edge>::iterator ds = sc_frame[i].f_slot[j].s_edge.begin();ds != sc_frame[i].f_slot[j].s_edge.end();ds++)
                        {
                            ...
                            if(ch.set_epsilon() < 1)
                            {
                                ...
                                cancellation_slot(device_number,slot_number, frame_number , sc_frame);
                                f_device[device_number].counter++;
                                
                                if((f_device[device_number].d_edge.size() > 0)&&(f_device[device_number].counter >= IS))
                                {
